...
public class AddReminder extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setReminder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String s=dateAndTimeInput.getText().toString();
                long timer=getTime(getYear(s),getMonth(s),getDay(s),getHour(s),getMinute(s), 0);
                if (dateAndTimeInput.getText().length() == 0) {
                    //do nothing
                } else {
                    setNotification(timer, title_input.getText().toString(), desc_input.getText().toString());
                }
    ...
    public void setNotification(long timer, String title, String desc) {
        long b = 30000L;
        ...
        if (timer < cutoff) {   //Any time that passed wont set reminder.
            ...
            PendingIntent pendingIntent = PendingIntent.getBroadcast(AddReminder.this, (int) timer, intent, PendingIntent.FLAG_MUTABLE);
            alarmManager.setInexactRepeating(AlarmManager.RTC_WAKEUP, timer, b, pendingIntent);
        }
    }
    public long dailyReminder(){
        long timer=86400000L;
        return timer;
    }
    public long weeklyReminder(){
        long timer=604800000L;
        return timer;
    }
    public long MonthlyReminder(){
        long timer=604800000L;
        return timer;
    }
    public long annualReminder(){
        long timer=604800000L;
        return timer;
    }
